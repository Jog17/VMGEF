...
export default function Home({ programs, events = [], featuredEvents, testimonials, homePageData }: HomeProps) {
  ...
  return (
    <main ref={container} className="w-full min-h-screen bg-vmgef-bg overflow-hidden">
      ...
      {/* FEATURED INITIATIVES (Items from Jahzara) */}
      {homePageData?.featuredInitiatives && homePageData.featuredInitiatives.length > 0 && (
        <section className="py-24 bg-vmgef-bg relative z-20 -mt-20">
          <div className="max-w-[96rem] mx-auto px-6 md:px-12">
            <div className={`featured-grid grid grid-cols-1 md:grid-cols-2 ${
              homePageData.featuredInitiatives.length === 1 ? 'lg:grid-cols-1 max-w-xl mx-auto' :
              homePageData.featuredInitiatives.length === 2 ? 'lg:grid-cols-2 max-w-5xl mx-auto' :
              homePageData.featuredInitiatives.length === 3 ? 'lg:grid-cols-3 max-w-7xl mx-auto' :
              'lg:grid-cols-4'
            } gap-6`}>
              {homePageData.featuredInitiatives.map((item: any, i: number) => {
