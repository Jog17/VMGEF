...
export default function About({ teamMembers, aboutPageData }: AboutProps) {
    ...
    <main ref={container} className="w-full min-h-screen bg-vmgef-bg pt-32">
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-12 mb-24 md:mb-40">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div>
            <span className="text-vmgef-orange tracking-[0.2em] uppercase text-sm font-semibold mb-6 block about-title">
              {aboutPageData?.hero?.subtitle || "Our Story"}
            </span>
            ...
            <p className="text-xl text-vmgef-ink-light font-light leading-relaxed about-title">
              {aboutPageData?.hero?.quote || '"Dedicated to the legacy of the best man I have ever known, my late son Vince."'}
            </p>
          </div>
          <div className="h-[500px] lg:h-[700px] overflow-hidden about-hero-img relative rounded-3xl">
            {aboutPageData?.hero?.image ? (
              <img 
                src={urlForImage(aboutPageData.hero.image)?.url() || ""} 
                alt="Hero image" 
      ...
      <section className="py-24 bg-white">
        <div className="max-w-4xl mx-auto px-6 md:px-12">
          <div className="fade-up mb-16">
            <h2 className="font-serif text-4xl md:text-5xl text-vmgef-ink mb-8">
      ...
      <section className="parallax-section relative h-[60vh] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0 z-0 bg-vmgef-ink">
          {aboutPageData?.parallax?.backgroundImage ? (
            <img 
              src={urlForImage(aboutPageData.parallax.backgroundImage)?.url() || ""} 
              alt="Education" 
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
          <h2 className="fade-up font-serif text-4xl md:text-6xl text-white leading-tight">
            {aboutPageData?.parallax?.quote || '"We provide tangible solutions to unemployment through a variety of impactful educational programs."'}
          </h2>
        </div>
      </section>
